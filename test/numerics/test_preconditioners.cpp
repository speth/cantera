...
TEST(PreconditionerTestSet, test_one_step_mechanism_network)
{
    ...
    int flag = network.FuncEval::preconditioner_setup_nothrow(startTime, y.data(), ydot.data(), gamma);
    // Creating external preconditioner for comparison
    AdaptivePreconditioner externalPrecon;
    ...
    EXPECT_EQ(externalPrecon==internalPrecon, true);
    internalPrecon.getMatrix()->setZero();
    network.preconditionerSetup(startTime, y.data(), ydot.data(), 1.0);
    EXPECT_EQ(externalPrecon==internalPrecon, true);
    EXPECT_EQ(flag, 0);
...
TEST(PreconditionerTestSet, test_initialize_set_dimensions)
{
...
TEST(PreconditionerTestSet, test_get_set_copy_assignment_compare)
{
    // Seed random number generator
    std::srand(std::time(NULL));
