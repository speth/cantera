...
:color: primary
Chemical equilibrium is not necessarily defined in terms of any reactions. For whatever
set of species are allowed in a phase or multi-phase mixture, it is defined as the
composition that minimizes the Gibbs free energy (for equilibrium at constant
temperature and pressure). This composition may be different from what you would get,
for example, from integrating a reactor network using a set of irreversible chemical
